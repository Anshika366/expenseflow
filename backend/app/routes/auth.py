...
async def signup(payload: UserSignupModel):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Database or Authentication Error: {str(e)}")
...
async def login(payload: UserLoginModel):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Database or Login Error: {str(e)}")
